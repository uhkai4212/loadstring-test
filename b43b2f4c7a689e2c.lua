...
local Window = OrionLib:MakeWindow({Name = "clock.lua - Mines", HidePremium = false, SaveConfig = true, ConfigFolder = "clock.lua.mines"})
...
local function findtradertom()
    if not tradertomPos then
        HRP.CFrame = CFrame.new(Vector3.new(998, 245, -71))
        local attempt = 0
        repeat
...
local function findNearestItem()
    if not items or not LPlayer.Character or not LPlayer.Character:FindFirstChild("HumanoidRootPart") then
        return nil
    end
...
local function MineOres()
    while AutoMine do
        ...
        elseif MiningDir == "Towards Ores" then
            ...
            if closestItem and LPlayer.Character and LPlayer.Character:FindFirstChild("HumanoidRootPart") then
                local playerPos = LPlayer.Character.HumanoidRootPart.Position
                local itemPos
...
local function MineOresDrill()
    while AutoDrill do
        ...
        Drill:FireServer(math.random(0,999), {direction = minePos, heat = 0, overheated = false})
        task.wait(0.05)
    end
end
...
local function UseDynamite()
    while AutoDynamite do
        ...
        Dynamite:FireServer(math.random(0,999), hitPosition)
        task.wait(0.5)
    end
end
...
local function navigateToNearestOre()
    ...
    local LPlayerHeadPos = LPlayer.Character.Head.Position
    local closestItem = findNearestItem()
    local targetPos = nil
    ...
    if targetPos then
        if closestItem then
            local rayOrigin = LPlayerHeadPos
            local rayDirection = targetPos - LPlayerHeadPos
            local raycastParams = RaycastParams.new()
            raycastParams.FilterDescendantsInstances = {LPlayer.Character, items}
            ...
            if success and path.Status == Enum.PathStatus.Success then
                for _, waypoint in ipairs(path:GetWaypoints()) do
                    if not LPlayer.Character or not LPlayer.Character.Humanoid then
                        break
                    end
...
local function SellInventory()
    ...
    local success, err = pcall(function()
        ...
        HRP.CFrame = CFrame.new(tradertomPos)
        task.wait(0.5)
