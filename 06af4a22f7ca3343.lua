            ...
            if game:GetService("Players").LocalPlayer.PlayerGui.loading.Enabled == false then
                for _, rod in pairs(workspace[playerName]:GetChildren()) do
                    if string.find(rod.Name,"Rod") then
                        ...
                        --     if auroraTotem and auroraTotem:FindFirstChild("ProximityPrompt") then
                        --         fireproximityprompt(auroraTotem.ProximityPrompt)
                        --     end
                        
                        --     -- ยืนยันการซื้อผ่าน GUI
                        --     local playerGui = game:GetService("Players")[playerName].PlayerGui
                        --     if playerGui and playerGui:FindFirstChild("over") and playerGui.over:FindFirstChild("prompt") then
                        --         local confirmButton = playerGui.over.prompt:FindFirstChild("confirm")
                        --         if confirmButton then
                        --             interact1(confirmButton)
                        --         end
                        --     end
